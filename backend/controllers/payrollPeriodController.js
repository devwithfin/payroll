...
const PayrollPeriodController = {
  ...
  createDraftPayroll: async (req, res) => {
    ...
    try {
      ...
      await PayrollDetail.destroy({ where: { period_id: periodId, payroll_status: "Draft" } });
      ...
      for (const emp of employees) {
        ...
        const totalOvertimePay = overtimes.reduce((sum, o) => sum + parseFloat(o.overtime_amount || 0), 0);
        ...
        const otherDeductions = deductions.reduce((sum, d) => sum + parseFloat(d.amount || 0), 0);

        const baseSalary = parseFloat(emp.position?.base_salary || 0);
  ...
  createFinalPayroll: async (req, res) => {
    ...
    try {
      ...
      for (const item of payrolls) {
        ...
        const pph21 = calculatePph21(base * 12, ptkp, brackets) / 12;

        const bpjs_kesehatan = base * parseFloat(config.bpjs_kesehatan_employee_rate || 0);
        const bpjs_jht = base * parseFloat(config.bpjs_tk_jht_employee_rate || 0);
        const bpjs_jp = base * parseFloat(config.bpjs_tk_jp_employee_rate || 0);
        const bpjs_jkm = base * parseFloat(config.bpjs_tk_jkm_employee_rate || 0);
